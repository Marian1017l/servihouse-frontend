...
const ProductStockDashboard = () => {
    const [products, setProducts] = useState([]);
        ...
        const fetchProducts = async () => {
                ...
                const dispatcherResp = await business.getDispatcherByUserId(userId);
                console.log(dispatcherResp);
                
                if (!dispatcherResp.success || !dispatcherResp.data) {
                    setLoading(false);
                    return;
                }
                const dispatcherId = dispatcherResp.data.id;
                console.log(dispatcherId);
                
                const stockResp = await business.getStockByDispatcher(dispatcherId);
                console.log(stockResp);
                
                if (!stockResp.success || !stockResp.data || stockResp.data.length === 0) {
                    setLoading(false);
                    return;
                }
                const storageId = stockResp.data[0].storage_id;
...
export default ProductStockDashboard;